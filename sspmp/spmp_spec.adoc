...
+
. `sstatus.SUM` should be WARL.
+
. `sstatus.MXR` should be WARL.
...
2. To better support M-mode emulation handler (e.g., the emulation handler needs to read instructions with `MXR=1` and `MPRV=1`), `sstatus.MXR` (Make eXecutable Readable) should be WARL and the semantics of `MXR` in SPMP are consistent with those of the Machine-Level ISA (see the "Memory Privilege in mstatus Register" subsection).
====
