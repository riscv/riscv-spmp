...
+
. When `spmpcfg.SHARED == 1`, the SUM bit is ignored when the *U bit must be 1*. The combination `SHARED=1, U=0` is reserved for future standard use.
+
...
Each combination of `siselect` and `sireg` represents an access to the corresponding S-level PMP entry CSR.
