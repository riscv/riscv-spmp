...
== S-level Physical Memory Protection (SPMP)
...
+
. The `Smstateen` extension must be implemented to support supervisor state enablement.
+
. The `sstatus.SUM` (permit Supervisor User Memory access) bit must be *writable*, which is a change from the Privileged Architecture. In SPMP, this bit modifies the privilege with which S-mode loads and stores access to physical memory. Hence, the need to make it writable.
...
// ____
+
. The `sstatus.MXR` (Make eXecutable Readable) bit must be *writable*, which is also a change from the Privileged Architecture. In SPMP, the `MXR` bit modifies the privilege with which loads access physical memory. Its semantics are consistent with those of the Machine Mode ISA. In SPMP, this bit is made writable to support M-mode emulation handlers where instructions are read with `MXR=1` and `MPRV=1`.
...
The layout within each SPMP configuration register is shown in <<spmpcfg>>. These registers are WARL.
...
. The SHARED bit marks a *Shared-Region* rule.
...
[NOTE]
====
The `spmpcfg[i]` is 16-bit. The low 8 bits of the SPMP configuration register alias to the 8 bits of the corresponding PMP configuration register field.
...
+
. When `spmpcfg.SHARED == 1`, the SUM bit is ignored and *the U bit must be 1*. The combination `SHARED=1, U=0` is reserved for future standard use.
+
...
=== Access Method of PMP_Entry CSRs

*Indirect CSR access*: `PMP_Entry` CSRs can be accessed indirectly from M-mode.
Each `miselect` represents an access to the corresponding M-level PMP entry CSRs.
The `mireg` accesses `spmpaddr`, and the `mireg2` accesses `spmpcfg`.
`mireg3`, `mireg4`, `mireg5`, and `mireg6` are read-only 0.

There is no ordering guarantee between writes to different `PMP_Entry` CSRs via indirect access.
Executing an `SFENCE.VMA` instruction with `rs1=x0` and `rs2=x0` orders all following implicit and explicit accesses with respect to all preceding writes to `PMP_Entry` CSRs.
...
[[access_method]]
=== Access Method of S-level PMP entry CSRs
*Indirect CSR access*: The S-level PMP entry CSRs are accessed indirectly.
...
Each `siselect` represents an access to the corresponding S-level PMP entry CSRs.
The `sireg` accesses `spmpaddr`, and the `sireg2` accesses `spmpcfg`.
`sireg3`, `sireg4`, `sireg5`, and `sireg6` are read-only 0.

// There is no ordering guarantee between writes to different S-level PMP entry CSRs.
// Executing an `SFENCE.VMA` instruction with `rs1=x0` and `rs2=x0` orders all following implicit and explicit accesses with respect to all preceding writes to S-level PMP entry CSRs.
...
Writes to `spmpcfg[i]` are right-justified. For example, when accessing `spmpcfg1` via `siselect` and `sireg2`, all 16 bits will be written right-justified.
====
