...
. The L bit marks an entry as locked, i.e., writes to the configuration register and associated address registers are ignored.
...
S-mode can only access bit `0-6` of each field of the configuration register.
This means that `spmp[i]cfg.L` can only be accessed by M-mode.

[cols="^1,^2",stripes=even, options="header"]
|===
|`siselect` number| indirect CSR access of `sireg`
|`siselect#1`|`sireg` -> `spmpaddr[0]`, `sireg2` -> `spmp[0]cfg{0..6}`
|`siselect#2`|`sireg` -> `spmpaddr[1]`, `sireg2` -> `spmp[1]cfg{0..6}`
|    ...     |    ...     
|`siselect#64`|`sireg` -> `spmpaddr[63]`, `sireg2` -> `spmp[63]cfg{0..6}`
|===

[NOTE]
====
The rationale for SPMP only assign one rule per siselect value is due to performance consideration.
If multiple SPMP rules are assigned to each `siselect`, a jump table or additional calculations would be needed to determine which `sireg` to assess.
