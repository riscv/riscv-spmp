...
The optional RISC-V S-mode Physical Memory Protection (SPMP) extension provides per-hart supervisor-mode control registers to allow physical memory access privileges (read, write, execute) to be specified for each physical memory region.
...
Similar to the PMP architecture, each SPMP entry is described by an 8-bit configuration register and one XLEN-bit address register. Some SPMP settings additionally use the address register associated with the preceding SPMP entry. Up to 64 SPMP entries are supported.
...
The SPMP address registers are CSRs named `spmpaddr0`-`spmpaddr63`, and uses the same encoding as the PMP architecture.
...
+
. A *Shared-Region* rule is specified by an `spmpcfg` setting with the R bit clear but the W bit set. This rule is enforced on both Supervisor and User modes, with restrictions depending on the `spmpcfg.S` and `spmpcfg.X` bits:
+
* If `spmpcfg.S` is unset, the region can be used for sharing data between S-mode and U-mode, but will disallow executable permission to both U-mode and S-mode. S-mode has RW permission to that region. U-mode has RW permission if `spmpcfg.X` is set, and it is restricted to read-only if `spmpcfg.X` is cleared.
+
* If `spmpcfg.S` is set, the region can be used for sharing code between S-mode and U-mode, but will disallow write permission to both U-mode and S-mode to that region. S-mode has RX permission to that region. U-mode has RX permission if `spmpcfg.X` is set, and it is restricted to execute-only if `spmpcfg.X` is cleared.
+
...
The `spmp[i]cfg.A` is WARL, so an implementation can hardwire the address matching method to a specific one it desires.
====
...
2. If the effective privilege mode of the access is S/U and no SPMP entry matches, but at least one SPMP entry is delegated, the access is `denied`; (Description of the mechanism of SPMP entry delegation is in <<PMP_Resource_Sharing>>).
