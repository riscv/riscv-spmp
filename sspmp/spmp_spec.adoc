...
=== Access Methods of PMP_Entry CSRs for M-mode
Delegated `PMP_Entry` CSRs are only accessible indirectly, while non-delegated `PMP_Entry` CSRs are only accessible directly.
...
[[access_method]]
=== Access Method of S-level PMP entry CSRs for S-mode
The S-level PMP entry CSRs are only accessible indirectly.
