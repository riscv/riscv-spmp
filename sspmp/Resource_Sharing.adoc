...
The remaining entries, `PMP entry[16]` to `PMP entry[63]`, would be mapped as `SPMP[0]` to `SPMP[47]`.
A read of an out-of-index PMP entry (e.g., `PMP[16]` or `SPMP[48]`) will return 0, and a write to such a PMP entry will be ignored.


// [NOTE]
// ====
// Software that uses SPMP should start with `SPMP[0]`.
// ====
