...
[NOTE]
====
Indirect accesses to SPMP CSRs are not ordered with respect to each other or to subsequent memory accesses (this allows fast context switching of SPMP registers).
...
Consequently, to synchronize preceding indirect writes to SPMP registers with subsequent memory accesses, software must execute an `SFENCE.VMA` instruction with rs1=x0 and rs2=x0.
====
