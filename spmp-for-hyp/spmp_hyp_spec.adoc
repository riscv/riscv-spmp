...
== "Ssvspmpmask" Extension for Optimizing Context Switching of SPMP Entries
...
For this introduces new virtual CSRs that control the activation of vSPMP entries:
...
== "Sshspmpmask" Extension for Optimizing Context Switching of hSPMP Entries

=== Extension Dependencies

. The `Sshspmpmask` is dependent on `Sshspmp` and `Sspmpsw`.

=== Optimization for Hypervisor/Guest Context Switching of SPMP Separation

The `Sshspmpmask` introduces additional CSRs:

* In RV64: one 64-bit WARL CSR called `hspmpmask` is added.
* In RV32: in addition to `hspmpmask`, a 32-bit WARL CSR called `hspmpmaskh` is added, which is an alias of the upper half of `hspmpmask`.

When V = 1, these registers replace the function of `spmpswitch`. Specifically, when V = 1, an hSPMP entry is considered active only if both its `spmpcfg[i].A` is set and the corresponding bit in `hspmpmask` is set. When V = 0, the `hspmpmask` register has no effect, and the baseline SPMP behavior applies as defined by `spmpswitch` alone.
...
Similarly, `spmpswitch` governs SPMP entries for the hypervisor’s own address space, while `hspmpmask` controls the guest address space enforced by hSPMP. This separation allows the hypervisor to manage its own and guest memory protection configurations independently.
====
